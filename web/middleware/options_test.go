...
// REST AutomaticOptions test
func testRESTOptions(r *http.Request, f func(*web.C, http.ResponseWriter, *http.Request)) *httptest.ResponseRecorder {
	var c web.C

	h := func(w http.ResponseWriter, r *http.Request) {
		f(&c, w, r)
	}
	m := RESTAutomaticOptions(&c, http.HandlerFunc(h))
	w := httptest.NewRecorder()
	m.ServeHTTP(w, r)
...
var restOptionsTestEnv = map[interface{}]interface{}{
	web.ValidMethodsKey: []string{
		"hello",
		"world",
		"rest",
...
func TestRESTAutomaticOptions(t *testing.T) {
	t.Parallel()

	// Shouldn't interfere with normal requests
	r, _ := http.NewRequest("GET", "/", nil)
	rr := testRESTOptions(r,
		func(c *web.C, w http.ResponseWriter, r *http.Request) {
			w.Write([]byte{'h', 'i'})
		},
	)
	...
	allow = rr.HeaderMap.Get("Allow")
	correctHeaders := "hello, world, rest, OPTIONS"
	if allow != "hello, world, rest, OPTIONS" {
		t.Errorf("Allow header should be %q, was %q", correctHeaders,
			allow)
	}
